(function () {
	...
	function Controller(ParticipantActivityService, ApplicationStateService, $mdDialog, DialogService, LoadingScreenService, $q, $timeout) {
		...
		function _loadActivityDtosfromStorage() {
			let resultFromStorage = window.sessionStorage.getItem('activityDtos');
			if (resultFromStorage) {
				let _activitiesFromStorage = []
				JSON.parse(resultFromStorage).forEach(result => {
					let survey = ParticipantActivityService.getSurveyFromJson(result.surveyForm.surveyTemplate);
